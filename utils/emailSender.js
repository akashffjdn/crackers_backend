...
const sendEmail = async (options) => {
  ...
  if (
    ...
    console.error(
      "CRITICAL: Email service configuration missing in .env file. Cannot send emails."
    );
...
<p>Thanks,<br>The ${process.env.EMAIL_FROM_NAME || "Sparkle Crackers"} Team</p>
...
`;
  ...
  const mailOptions = {
    ...
    from: `"${process.env.EMAIL_FROM_NAME || "Akash Crackers Support"}" <${
    ...
    html:
      options.htmlMessage ||
      generateHtmlContent(options.subject, options.message, options.resetUrl), // Added HTML template usage
  };
  ...
  try {
    console.log(
      `Attempting to send email to ${options.email} via ${process.env.EMAIL_HOST}...`
    );
    ...
    throw new Error(
      "Email could not be sent due to a server configuration or network error."
    );
  }
};
